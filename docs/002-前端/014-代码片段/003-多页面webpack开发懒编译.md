```js
const execa = require("execa")
const express = require("express")
const server = express()
const proxy = require("http-proxy-middleware")
const path = require("path")
const inquirer = require("inquirer")

let port = 6500
const pages = require('../src/page').map(page => {
  return {
    name: page.pageName,
    title: page.title,
    port: port++,
    isStart: false
  }
})

function compile(page) {
  page.isStart = true
  execa(
    `webpack-dev-server --inline  --config build/webpack.dev.conf.js --env.pageName ${page.name} --env.port ${page.port}`, [], {
      stdio: 'inherit'
    }
  )
}

const promptList = [{
  type: "checkbox",
  message: "选择要编译的页面(未选择的页面将在访问时编译): ",
  name: "pages",
  choices: pages
}];

inquirer.prompt(promptList).then(selectPages => {
  const pageMap = {}
  pages.forEach(page => {
    pageMap[page.name] = page
  })
  selectPages.pages.forEach(pageName => {
    const page = pageMap[pageName]
    if (page) {
      compile(page)
    }
  })

  server.listen(8080, err => {
    if (err) {
      console.log(err)
    } else {
      console.log("当前采用懒编译模式，只有在访问时才会进行编译。")
    }
  })
})

server.use(express.static(path.resolve(process.cwd(), "client")))
server.use(express.static(path.resolve(process.cwd(), "src")))

server.use("*", proxy({
  target: '  ',
  changeOrigin: true,
  ws: true,
  router: function (req) {
    for (let page of pages) {
      if (req.url === `/${page.name}` || req.headers['accept'].indexOf('text/html') < 0 && new RegExp(`${page.name}$`).test(req.headers.referer)) {
        if (!page.isStart) {
          setTimeout(() => {
            compile(page)
          })
        }
        return `http://localhost:${page.port}`;
      }
    }
    return `http://localhost:8080`
  },
  onError(err, req, res) {
    res.writeHead(200, {
      'Content-Type': 'text/html; charset=UTF-8'
    });
    if (err.code === "ECONNREFUSED") {
      res.end(`<!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8" />
        <style>
          html, body, iframe {
            margin: 0;
            padding: 0;
          }
          body {
            padding-top: 50px;
            text-align: center;
          }
          img {
            height: 200px;
          }
          h1 {
            color: #333;
          }
          div {
            color: #666;
          }
        </style>
      </head>
      <body>
        <img src="data:image/png;base64,"/>
        <h1>正在编译...(<span id="time">0</span>s)</h1>
        <div>当前页面采用懒编译模式，只有在访问时才会进行编译。</div>
        <div>每个页面第一次编译需要一点儿时间，请稍等一会儿~~~</div>
        <script>
        var oTime = document.getElementById("time")
        var time = 0
        setInterval(() => {
          location.reload()
          oTime.innerText = time++
        }, 1000);
        </script>
      </body>
      </html>`);
    } else {
      res.end(JSON.stringify(err));
    }
  }
}))

```